<script lang="ts">
	...
	let columns: number = sel?.columns ?? 2;
	...
	$: options = [
		...$entityList('light'), ...$entityList('switch'),
		...$entityList('input_boolean'), ...$entityList('fan'), ...$entityList('cover')
...
{#if isOpen}
	<Modal>
		<h1 slot="title">Button Group</h1>

		<h2>Title (optional)</h2>
		<InputClear condition={title} on:clear={()=>{title='';set('title');}} let:padding>
			<input type="text" class="input" bind:value={title} placeholder="My Devices" on:change={(e)=>set('title',e)} autocomplete="off" spellcheck="false" style:padding />
		</InputClear>

		<h2>Columns</h2>
		<div class="button-container">
			{#each [2, 3] as c}
				<button class:selected={columns===c} on:click={()=>{columns=c;sel.columns=c;$dashboard=$dashboard;}} use:Ripple={$ripple}>{c}</button>
			{/each}
		</div>

		<h2>Entities (max 6)</h2>
		<div class="list">
			{#each entityIds as eid, i}
				<div class="row">
					<div class="sel"><Select computeIcons={true} {options} placeholder="switch.* / light.*" value={eid} on:change={(e)=>{entityIds[i]=e.detail;saveIds();}} /></div>
			...
			{#if entityIds.length < 6}
				<!-- svelte-ignore a11y-click-events-have-key-events -->
				<!-- svelte-ignore a11y-no-static-element-interactions -->
				<button class="add" on:click={add} use:Ripple={$ripple}>+ Add entity</button>
			{/if}
		</div>
