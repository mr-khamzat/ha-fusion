<script lang="ts">
	...
	$: options = [...$entityList('sensor'), ...$entityList('binary_sensor'), ...$entityList('input_number')].sort((a,b)=>a.label.localeCompare(b.label));
	onDestroy(() => $record());
</script>

{#if isOpen}
	<Modal>
		<h1 slot="title">Sensor Grid</h1>

		<h2>Title (optional)</h2>
		<InputClear condition={title} on:clear={() => { title = ''; set('title'); }} let:padding>
			<input type="text" class="input" bind:value={title} placeholder="My Sensors" on:change={(event) => set('title', event)} autocomplete="off" spellcheck="false" style:padding />
		</InputClear>

		<h2>Sensors (up to 6)</h2>
		<div class="list">
			{#each entityIds as eid, i}
				<div class="row">
					<div class="sel"><Select computeIcons={true} {options} placeholder="sensor.*" value={eid} on:change={(e) => { entityIds[i]=e.detail; saveIds(); }} /></div>
			...
			{#if entityIds.length < 6}
				<!-- svelte-ignore a11y-click-events-have-key-events -->
				<!-- svelte-ignore a11y-no-static-element-interactions -->
				<button class="add" on:click={add} use:Ripple={$ripple}>+ Add sensor</button>
			{/if}
		</div>
