<script lang="ts">
	...
	import Sun from '$lib/Sidebar/Sun.svelte';
	import Todo from '$lib/Sidebar/Todo.svelte';
	import MiniPlayer from '$lib/Sidebar/MiniPlayer.svelte';
	import Gauge from '$lib/Sidebar/Gauge.svelte';
	import CalendarWidget from '$lib/Sidebar/CalendarWidget.svelte';
	import Thermostat from '$lib/Sidebar/Thermostat.svelte';
	import SolarFlow from '$lib/Sidebar/SolarFlow.svelte';
	import AnalogClock from '$lib/Sidebar/AnalogClock.svelte';
	import Battery from '$lib/Sidebar/Battery.svelte';
	import Markdown from '$lib/Sidebar/Markdown.svelte';
	import AlbumArt from '$lib/Sidebar/AlbumArt.svelte';
	import AutomationToggle from '$lib/Sidebar/AutomationToggle.svelte';
import HomeModeWidget from '$lib/Sidebar/HomeModeWidget.svelte';
import NetworkMonitor from '$lib/Sidebar/NetworkMonitor.svelte';
	import MyDay from '$lib/Sidebar/MyDay.svelte';
	...
	if (!$demo.todo) $demo.todo = getTodoEntity($states);
	...
	onMount(async () => {
		...
		// Defer widget render: flush pending Svelte updates first (tick),
		// then wait one animation frame so the modal paints before loading
		// all widget previews. Works even when isOpen=false on mount (replace flow).
		await tick();
	...
	$: itemTypes = [
		...
		{
			id: 'person',
			type: $lang('person') || 'Person',
			component: Person,
			props: {
				sel: { entity_id: $demo.person }
			}
		},
		{
			id: 'todo',
		...
		{
			id: 'mini_player',
			type: $lang('mini_player') || 'Mini Player',
		...
		},
		{
			...
			type: 'Text / Markdown',
			component: Markdown,
			props: { sel: { text: '<b>Hello</b> World' } }
		},
		{
			id: 'album_art',
			type: 'Album Art',
			component: AlbumArt,
			props: { sel: { entity_id: $demo.media_player } }
		},
		{
			id: 'automation_toggle',
			type: 'Automation Toggles',
			component: AutomationToggle,
			props: { sel: {} }
		},
		{
			id: 'home_mode_widget',
			type: 'Режим дома',
			component: HomeModeWidget,
			props: { sel: {} }
		},
		{
			id: 'network_monitor',
		...
		},
		{
			id: 'yandex_weather',
			type: 'Яндекс Погода',
			component: YandexWeather,
			props: { sel: { entity_id: $demo.weather } }
		}
	];
...
{#if isOpen}
	<Modal size="large">
		...
		<div class="container">
			{#each (showWidgets ? filter : []) as { id, type, component, props, style } (id)}
				<button
